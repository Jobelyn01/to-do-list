...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:20px_20px] flex flex-col items-center p-6 md:p-12 text-slate-800 font-sans">
      ...
      {/* NOTIFICATION TOAST */}
      {msg.text && (
        <div className={`fixed top-10 left-1/2 -translate-x-1/2 z-[200] px-8 py-4 rounded-2xl shadow-2xl font-bold text-white whitespace-nowrap flex items-center gap-2 ${msg.type === 'error' ? 'bg-rose-500' : 'bg-slate-800'} animate-bounce`}>
      ...
      <div className="w-full max-w-2xl space-y-3">
        {lists.length === 0 ? (
          <p className="text-center py-10 text-slate-400 font-medium italic">No boards found. Create your first one above!</p>
        ) : (
          lists.map(l => (
            <div key={l.id} className="flex justify-between items-center p-5 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all">
              ...
              <div className="flex gap-4 ml-4 text-xs font-black text-slate-300">
                <button onClick={() => { setEditingId(l.id); setEditingTitle(l.title); }} className="hover:text-slate-600 transition-colors">EDIT</button>
                <button onClick={() => { setListToDelete(l.id); setShowDeleteModal(true); }} className="hover:text-rose-400 transition-colors">DELETE</button>
              </div>
            </div>
          ))
        )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center">
            ...
            <p className="text-slate-500 mb-8 font-medium">This board and all its tasks will be permanently deleted.</p>
