...
import api from "./api"; // Siguraduhin na ang api.js at App.jsx ay magkasama sa 'src' folder
...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Dito natin tinatanggal ang 'undefined' message
      const errMsg = err.response?.data?.message || "Maling login details.";
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] flex items-center justify-center p-6 font-sans">
      ...
      <div className="w-full max-w-md bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100">
        <h1 className="text-4xl font-black text-center text-slate-900 mb-8">Focus Hub</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg mt-4 shadow-lg hover:bg-black transition-all">
            {isRegistering ? "Register" : "Login"}
          </button>
        </form>
        <button onClick={() => setIsRegistering(!isRegistering)} className="w-full mt-6 text-slate-500 font-medium hover:underline">
          {isRegistering ? "May account na? Login" : "Wala pang account? Sign up"}
        </button>
      </div>
    </div>
  );
